\chapter{Multi-objective problems}\label{multi-objective-chapter}
...
\end{example}
\section{Worst case guarantee} \label{worst-case-guarantee}
...
%The set of edges of this graph is included in $(V_1 \times V_2) \cup (V_2 \times V_1)$, where
% Each edge of this graph represents an alternation between a vertex of player one and a vertex of player two.
% The principle of such a turn-based game is that, at each step, the game is in a vertex belonging to a player, and this player chooses an edge leading this vertex to a vertex of the other player.\\
...
We compute the shortest path from $s$ ensuring to reach $T$ by dynamic programming in order to build a strategy satisfying the \SPG{} problem.
...
%we determine the value of $\mathbb{C}$ for each entry $(s, i) \in S \times \{0, \dots, n-1\}$ by looking at each $\alpha$-successor of $s$, for each $\alpha \in A(s)$, causing a worst case time in $\mathcal{O}(|\mathcal{M}|)$.
...
%We solve the problem by considering the unfolding of a given MDP $\mathcal{M}$ up to the worst case threshold $\ell_1$ and the set of target states $T$ as a game in order to ensure a worst case guarantee.
...
\begin{notation}[\textit{Attractors of a set of target states in an MDP}]
...
%In Chapter \ref{model-checking-chapter} (cf. Subsection \ref{temp-event-mdp}), we presented a way to compute $\Attr(T)$ in any MDP $\mathcal{M}$.
%In this section, we explicit a method to compute this attractor in an unfolded MDP, and furthermore
...
\begin{enumerate}
  ...
  \begin{itemize}
    ...
    \item $\mathbb{A}^*$ is the set of actions of this unfolding such that, for all action $\alpha \in A$, $\alpha \in \mathbb{A}^*$, and the set of enabled actions of each state $(s, v) \in S^\mathbb{A}_{\ell_1}$ is given by %$\mathbb{A}(s, v)$ if $\mathbb{A}(s, v) \neq \emptyset$ and a singleton containing an arbitrary action $\alpha \in A(s)$ else,
    \[
      ...
      \end{cases}
    \]
    If $\mathbb{A}(s, v) = \emptyset$, then $(s, v) \in T_{\ell_1}$, and we make it absorbing with one of its enabled action $\alpha$.
    \item $\Delta^*_{\ell_1}$ is defined the same way as for the classical unfolding $\mathcal{M}_{\ell_1}$,
    except for %$(s, v) \in S^\mathbb{A}_{\ell_1}$ such that $\mathbb{A}(s, v) = \emptyset$, where $\Delta^*_{\ell_1}((s, v),\, \alpha, \, (s, v)) = 1$ and $\Delta^*_{\ell_1}((s, v), \, \alpha, \, (s', v')) = 0$ if $(s', v') \neq (s, v)$, for $\alpha \in \mathbb{A}^*(s, v)$,
    the states $(s, v) \in T_{\ell_1}$ such that $\mathbb{A}(s, v) = \emptyset$. In that case, we make it absorbing with $\Delta^*_{\ell_1}((s, v),\, \alpha, \, (s, v)) = 1$ and $\Delta^*_{\ell_1}((s, v), \, \alpha, \, (s', v')) = 0$, with $\{\alpha\} = \mathbb{A}^*(s)$.
...
\begin{lemma}[Correctness of Algorithm \ref{safe-actions-algorithm}]\label{attr-algo-lemma-1}
...
%as the underlying graph of $\mathcal{M}_{\ell_1}$ has no cycle if we remove $s_\bot$ from it (the resulting graph is a DAG), and as we only consider predecessors of states at each iteration, we end up in the worst case with the ``root" of this DAG, and the algorithm stops.
...
\begin{lemma}[Time complexity of Algorithm \ref{safe-actions-algorithm}]\label{attr-algo-lemma-2}
 Computing $\mathbb{A}(s, v)$ for all $(s, v) \in S_{\ell_1}$ can be done in linear time in $\mathcal{M}_{\ell_1}$.
\end{lemma}

\begin{proof2}
% The maximum number of iteration of the outermost loop is $|S_{\ell_1}|$, since a state can be tagged as bad at most once.
% Then, the rest of operations corresponds to
% %looking at each predecessors of bad states in order to remove all actions from
% remove the actions leading to each bad states, causing a worst case time in $\mathcal{O}|\mathcal{M}_{\ell_1}|$ (each action can be removed at most once).
% First, in worst case, we tag all states as bad, which exactly corresponds to $|S_{\ell_1}|$ operations. States cannot be tagged more than once as bad, since the underlying graph of
% Then, we iteratively remove all unsafe actions from $\mathbb{A}$, by looking at the predecessors of each bad state.
% We assume for each bad state $(s', v')$ that we do not consider a predecessor $(s, \alpha) \in Pred((s', v'))$ if the action $\alpha$ has been removed from $\mathbb{A}(s', v')$.
% Thus, we don't look twice at a transition since we remove each action $\alpha$ of each predecessor $(s, \alpha)$ visited.
% Then, we look at all transitions of the system at most once that exactly corresponds to $|\{(s, \alpha, s') \in S_{\ell_1} \times A_{\ell_1} \times S_{\ell_1} \; | \; \Delta_{\ell_1}(s, \alpha, s') > 0 \}|$ operations.
% Adding up these two number of wost case operations, we get a a time complexity in $\mathcal{O}(|\mathcal{M}_{\ell_1}|)$.
%
% In line \ref{attr4}, we force the first iteration of the outermost loop \ref{attr14} to consider $s_\bot$.
% As $\mathbb{A}(s_\bot) = \emptyset$,
% %we only consider successors of $\mathcal{M}$, and
...
\begin{example}[\textit{\SSPWE{} problem in the wireless sensor network}]\label{sspwe-example}
...
\begin{notation}[\textit{Coordinate-wise inequality}]
...
% and $\mathcal{Q}_{s^*, r}$ be the \MOSR{} problem for the state $s^* \in S$ and the $r \in \mathbb{N}$ constraints described by multiples target sets $T_i \subseteq S$, for $i \in \{1, \dots, r\}$, where every target $t \in \bigcup_{i=1}^r T_i$ is an absorbing state, and probability thresholds $\alpha \in ([0, 1] \cap \mathbb{Q})^r$.
  Let $\mathcal{M}$ be an MDP with state space $S$,
  $s^* \in S$, multiples target sets $T_i \subseteq S$, for $i \in \{1, \dots, r\}$, where every target $t \in \bigcup_{i=1}^r T_i$ is an absorbing state, and a vector of probability thresholds $\alpha \in ([0, 1] \cap \mathbb{Q})^r$.
...
\begin{definition}[\textbf{\SSPPQ{} problem}]\label{ssppq-example-1}
...
\begin{example}[\textit{\SSPPQ{} problem in the wireless sensor network}]\label{final-example}
  ...
  \end{cases}
  \]
  for $s \in S'$ and $S'$, $A'$ being respectively state and action space of $\mathcal{M}'$.
